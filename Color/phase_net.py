...
class PhaseNet(nn.Module):
    """
    Input:  (B, 1, H, W) target amplitude image
    ...
    in_channels defaults to 1 (a plain amplitude target). For depth-aware
    training, pass in_channels=2: channel 0 is the content amplitude,
    channel 1 is a depth map -- the RGB-D idea, simplified to grayscale.

    out_channels defaults to 1 (one phase map). For color, pass
    out_channels=3: one independent phase map per wavelength, matching a
    three-panel full-color SLM setup.
    """
