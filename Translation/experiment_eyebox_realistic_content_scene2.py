"""
SECOND real-content scene, checking whether the reversal found on the
first photo (10.13: discrete-tile wins on average quality on real content,
7.28->9.19dB across the two real tests so far, vs. a near-tie on
procedural content) generalizes, or was specific to that photo's bright,
concentrated signage. This photo (IMG_2289) is structurally different on
purpose: a wrought-iron fence very close to the camera, an etched (not
backlit) metal plaque, and a church building + trees much further back --
no bright glowing signage, a bigger foreground/background depth gap, and
finer, lower-contrast near-field detail (the ironwork scrollwork).

Same pipeline and exact same confirmed condition as
experiment_eyebox_realistic_content.py (wide range MAX_U_FAR_PX=10, 4-tile
budget, 500-step training budget, Adam optimizer, per-pixel content mask)
-- only the content changes, built by build_second_scene_target.py
(no translation caption this time; not essential to this generalization
check).

Run on your 3060 (expect ~2-3 min for 6 seeds):
    python3 experiment_eyebox_realistic_content_scene2.py
"""
import time
import numpy as np
import torch
...
def load_base_planes():
    return [
        np.load(r"D:\Documents\CGH\Translation\target_near_2289.npy"),
        np.load(r"D:\Documents\CGH\Translation\target_mid_2289.npy"),
        np.load(r"D:\Documents\CGH\Translation\target_far_2289.npy"),
    ]
